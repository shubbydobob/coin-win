...
> **Phase 4 에서 드러난 것 — 완료 조건에 사람 손이 한 번 필요하다.**
> 트레이딩뷰 값은 코드로 가져올 수 없다. 그래서 golden test 의 기준값은 *닫힌 식이 나오는
> 합성 캔들*(등차수열의 모집단 분산, 단조 증가 캔들의 이동 최대·최소)과 *극값이 구간 중간에
> 오는 비단조 13봉*에서 얻는다. 근거는 `docs/adr/015`. 이 테스트들이 증명하는 것은 "구현이
> 명시된 공식을 정확히 따른다" 까지이고, **공식 해석이 트레이딩뷰와 같은지는 증명하지 않는다.**
>
> 남은 것은 **BTCUSDT 차트 대조 한 번**이다. 갈리는 지점은 일목 변위다 — 문서는 26봉이라
> 적지만 트레이딩뷰 내장 지표가 현재 봉을 1 로 세어 25 를 쓴다는 설명이 널리 퍼져 있고 공식
> 문서로 확인되지 않았다. 그래서 변위를 `IchimokuCloud` 의 설정 필드로 뒀다. 어긋나면 숫자
> 하나만 바꾼다. 대조 결과는 `docs/adr/014` 에 기록한다.
>
