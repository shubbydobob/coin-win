...
**변위 26 은 실제로 25봉을 민다.** 트레이딩뷰 Pine 소스가
`plot(leadLine1, offset = displacement - 1)` 이다. 입력 어휘는 트레이딩뷰와 맞춰 26 으로 두고
실제 이동은 `shift() = displacement − 1` 이 계산한다. 후행스팬도 대칭으로 같은 25 다
(`offset = -displacement + 1`). 근거는 `docs/adr/014` 에 원문을 인용해 두었다.

### 워밍업

첫 값이 나오는 index 는 일목이 `shift + 선행스팬2 기간 − 1`(기본 25 + 51 = 76, 즉 캔들 77개),
볼린저가 `기간 − 1`(19).
...
- 선행스팬 1 만 예외적으로 두 선의 평균이고, 나머지 셋은 전부 같은 형태다 —
  ...
  트레이딩뷰의 `donchian(len) => math.avg(ta.lowest(len), ta.highest(len))` 와 같다.
- 미래 구름(마지막 캔들 이후 25봉)은 내지 않는다. 근거는 `docs/adr/014`.
- 두 지표의 공식은 트레이딩뷰가 배포하는 **Pine 소스 원문과 대조해 확정**했다.
  값 몇 개를 눈으로 맞대는 것보다 강하다 — 모든 입력에 대해 성립하기 때문이다.
...
**결과를 구간으로 가를 때 캔들을 잘라 다시 돌리지 않는다.** 지표 워밍업이 구간마다
되살아나 각 구간 앞부분의 거래가 통째로 사라진다(4시간봉 일목 워밍업만 77봉, 열이틀).
룩어헤드의 **반대편**이라 안전해 보이지만 표본을 얇게 만드는 쪽으로 왜곡한다. 전체에서
한 번만 돌리고 **진입 시각**으로 가른다 — 엔진이 인과적이라 그 가르기가 성립한다.
청산 시각으로 가르면 경계를 걸친 거래가 아직 시작하지도 않은 구간에 들어간다.

**고르기와 검증을 한 메서드가 하지 않는다.** `choose` 는 학습 구간만 보고, 검증은
`outOfSampleOf` 로 따로 꺼낸다. 한 구간에서 격자를 돌려 최고를 집는 것은 측정이 아니라
고르기이고, 고른 구간에서 좋은 것은 정의상 언제나 좋다.

**고를 자격을 잃는 경우가 둘이다.** 학습 구간 거래가 `minimumTrades` 미만이면 그 성적은
전략의 성질이 아니라 몇 번의 우연이다. 진 거래가 하나도 없으면 손익비를 말할 수 없고,
그것을 무한대로 치면 **표본이 얇을수록 이기는** 고르기가 된다. 동점이면 먼저 온 후보를
고른다 — 순서가 흔들리면 같은 입력이 같은 표를 내지 않는다.

**측정 결과는 `docs/adr/021`.** 7년 이력에서 18조합 전부 손익비 1 미만이고, 워크포워드
5폴드 전부 검증에서 무너진다. 이 전략을 실계좌 후보로 취급하지 않는다.
