...
### 5.4bis 롱·숏 후보가 동시에 설 때 — 초안을 폐기했다

초안은 "둘 다 버린다" 고 적었다. **그 규칙은 모든 거래를 없앤다.** 가격은 거의 항상 어떤
지지 위에 있으면서 동시에 어떤 저항 아래에 있기 때문이다 — 그것이 예외 상황이 아니라 기본
상태다.

**근단이 현재 종가에 더 가까운 쪽 하나만 무장한다.** 지정가는 가까운 쪽이 먼저 닿고, 동시
포지션이 1개이므로 어차피 하나만 살아남는다. 거리가 정확히 같으면 그때는 고를 근거가 없으므로
버린다.

둘 다 무장하고 "먼저 체결되는 쪽이 이긴다" 로 두는 선택지도 있었으나, 한 봉 안에서 양쪽
지정가가 모두 닿는 경우에 순서를 정할 근거가 다시 없어진다. § 6 의 "보유자에게 불리한 쪽"
원칙은 이미 포지션이 있을 때의 규칙이라 여기에는 쓸 수 없다.
...
### 12.3 파라미터 기본값 — 닫혔다. 바꾸지 않기로 했다

실제 BTCUSDT 4시간봉 1,500개로 18조합을 돌렸다(`.\gradlew.bat crossCheck`). **15조합의 손익비가
1 미만이었다.** 최고 조합은 5 / 1.0 / 0.5 (손익비 1.33, 33건)인데 인접 조합 5 / 1.0 / 1.0 이
0.99 로 뒤집힌다.

**그래서 기본값을 바꾸지 않는다.** 표의 최고 조합을 박는 것이 과최적화의 정의다. 33건 표본에서
나온 숫자를 전략의 성질로 착각하게 만든다. `ZoneSettings.standard()` 는 5 / 0.5 / 2 / 14 로
남는다.

구조적으로 읽히는 것만 기록한다 — 손절 버퍼 0.5 ATR 은 낙폭을 32~90% 로 키우고 1.0 은 8~29% 로
막는다(대가 뚫린 것이 아니라 꼬리에 스친 것을 손절로 세고 있다). 피벗 8봉은 일관되게 나쁘다.
지표 필터는 거래를 절반 줄이며 손익비를 올리지만 **12건은 통계가 아니다.**

전문은 `.claude/docs/roadmap.md` Phase 6 절.
